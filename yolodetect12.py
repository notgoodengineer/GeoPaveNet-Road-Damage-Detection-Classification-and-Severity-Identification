...
def get_usb_path():
    """Find real USB drives (exclude Docker container dirs)"""
    usb_mounts = glob.glob("/media/geopavenet/*")
    
    # Filter criteria (adjust as needed):
    # 1. Is a directory
    # 2. Not a UUID-like name (Docker containers use these)
    # 3. Not hidden/system folder
    valid_usbs = [
        d for d in usb_mounts
        if (os.path.isdir(d) and 
            not re.match(r'^[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}\d*$', os.path.basename(d)) and
            not d.startswith('.'))
    ]
    
    if not valid_usbs:
        raise RuntimeError("No USB drive detected. Insert a USB and try again.")
    
    return valid_usbs[0]  # Use first valid USB

# Configure paths
USB_BASE = get_usb_path()
DETECTIONS_DIR = os.path.join(USB_BASE, "detections")
IMAGES_DIR = os.path.join(DETECTIONS_DIR, "images")
DB_DIR = os.path.join(DETECTIONS_DIR, "database")
...
DATABASE_PATH = os.path.join(DB_DIR, "detections.db")
...
def address_retry_worker(retry_interval=60):
    ...
    while True:
        try:
            ...
            cursor.execute('''
                ...
                FROM detections 
                WHERE address = 'Address lookup pending'
                   OR address = 'Address lookup error'
                   OR address = 'Database error'
            ...
            for entry in pending_entries:
                ...
                try:
                    ...
                    ''', (address, entry_id))
                    conn.commit()
                    print(f"Updated address for entry {entry_id}")
                    time.sleep(1)  # Respect Nominatim's rate limit
                except Exception as e:
                    print(f"Failed to update address for entry {entry_id}: {e}")
                    # Mark as error if failed, will retry next time
                    cursor.execute('''
                        UPDATE detections 
                        SET address = 'Address lookup error'
                        WHERE id = ?
                    ''', (entry_id,))
                    conn.commit()
            
        except Exception as e:
            print(f"Address retry worker error: {e}")
        finally:
...
def reverse_geocode(lat, lon):
    ...
    for attempt in range(5):  # Increased retry attempts
        try:
            ...
            print(f"Geocoding attempt {attempt+1}/5 failed: {e}")
...
def cleanup_database(conn):
    cursor = conn.cursor()
    # Existing orphaned image cleanup
    ...
    cursor.execute('''
        SELECT id, latitude, longitude 
        FROM detections 
        WHERE address IN ('Address lookup error', 'Address not found', 'No GPS data')
          AND latitude IS NOT NULL 
          AND longitude IS NOT NULL
        LIMIT 10  
    ''')
    retry_entries = cursor.fetchall()
    
    for entry in retry_entries:
        entry_id, lat, lon = entry
        address = reverse_geocode(lat, lon)
        if address not in ["Address lookup error", "Address not found", "No GPS data"]:
            print(f"Updating address for entry {entry_id}")
            cursor.execute('''
                UPDATE detections 
                SET address = ?
                WHERE id = ?
...
def db_cleanup_worker(interval=30):
    conn = init_database()
    while True:
...
def db_worker(db_queue):
    ...
    while True:
        ...
        # Always save the detection immediately with pending status
        address = "Address lookup pending" if lat is not None and lon is not None else "No GPS data"
        
        try:
            cursor.execute('''
                INSERT INTO detections 
                (timestamp, latitude, longitude, address, classification, severity, confidence, image_path)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (db_timestamp, lat, lon, address, classification, severity, confidence, image_path))
            conn.commit()
            print(f"Inserted detection into database (address status: {address})")
        except sqlite3.Error as e:
            print(f"Database error: {e}")
            # Try one more time with simplified data if first attempt failed
            try:
                cursor.execute('''
                    INSERT INTO detections 
                    (timestamp, latitude, longitude, address, classification, severity, confidence, image_path)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (db_timestamp, lat, lon, "Database error", classification, severity, confidence, image_path))
                conn.commit()
            except sqlite3.Error as e:
                print(f"Failed to insert into database after retry: {e}")
...
def generate_report():
    try:
        ...
        os.chown(report_path, os.getuid(), os.getgid())
...
def main():
    ...
    address_retry_thread = threading.Thread(target=address_retry_worker, daemon=True)
    address_retry_thread.start()
        ...
        if is_detecting:
            ...
            for box in results[0].boxes:
                ...
                if confidence > CONFIDENCE_THRESHOLD:
                    ...
                    filename = os.path.join(IMAGES_DIR, f"{filename_timestamp}_{class_name}_{unique_id}.jpg")
