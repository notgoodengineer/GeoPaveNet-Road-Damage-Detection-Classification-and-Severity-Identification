...
def address_retry_worker(retry_interval=180):
    ...
    while True:
        try:
            ...
            cursor.execute('''
                ...
                FROM detections 
                WHERE address = 'Address lookup error'
            ...
            for entry in failed_entries:
                ...
                if address != "Address lookup error":
                    cursor.execute('''
                        UPDATE detections 
                        SET address = ?
                        WHERE id = ?
...
def cleanup_database(conn):
    ...
    # New: Retry failed address lookups
    cursor.execute('''
        SELECT id, latitude, longitude 
        FROM detections 
    ...
    while True:
        ...
        if lat is not None and lon is not None:
            ...
            if address is None:
                ...
                if "error" not in address.lower() and "not found" not in address.lower():
