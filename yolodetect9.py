...
def get_street_address(lat, lon, max_retries=2):
    """Get street-level address (online or offline)"""
    if lat is None or lon is None:
        return "Unknown location"
    
    # --- Try Online (Nominatim) ---
    try:
        geolocator = Nominatim(user_agent="road_damage_detector")
        ...
        print(f"Online geocoding failed: {e}")
    
    # --- Offline Fallback ---
    try:
        result = rg.search((lat, lon))[0]
        return f"{result.get('name', 'Unknown')}, {result.get('admin2', '')}, {result.get('cc', '')}"
    except Exception as e:
...
def db_worker(db_queue):
    """Robust database worker with error handling and address processing"""
    conn = None
    max_retries = 3
    retry_delay = 1
    ...
    while True:
        try:
            # Initialize/reconnect to database
            if conn is None:
            ...
            # Process queue items
            try:
                data = db_queue.get(timeout=5)  # Add timeout to prevent hanging
                
                if data is None:  # Shutdown signal
                    print("Database worker received shutdown request")
                    break
                    
                # Validate and unpack data
                if len(data) != 7:
                    print(f"Invalid data format (expected 7 items, got {len(data)}): {data}")
                    db_queue.task_done()
                    continue
                    
                timestamp, lat, lon, classification, severity, confidence, image_path = data
                
                # Get address with error handling
                try:
                    address = get_street_address(lat, lon) if (lat is not None and lon is not None) else "Unknown location"
                except Exception as e:
                    print(f"Geocoding failed: {e}")
                    address = f"Coordinates: {lat:.6f}, {lon:.6f}" if (lat and lon) else "Unknown location"
                
                # Database insertion with retries
                    ...
                    try:
                        ...
                        cursor.execute('''
                            ...
                            (timestamp, latitude, longitude, address, 
                             classification, severity, confidence, image_path)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        ''', (timestamp, lat, lon, address, 
                              classification, severity, confidence, image_path))
                        conn.commit()
                        print(f"Successfully saved to DB: {image_path}")
                        break
            ...
            if 'data' in locals():  # Only if we got data
                db_queue.task_done()
            time.sleep(retry_delay)
        ...
        if is_detecting:
            ...
            for box in results[0].boxes:
                ...
                if confidence > CONFIDENCE_THRESHOLD:
                    ...
                    # Generate address with fallback
                    ...
                    db_queue.put((db_timestamp, lat, lon, address, classification, severity, confidence, filename))
